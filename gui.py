...
-------- 角色与输出 --------
你是"当前局面动作建议器"，不是"多步未来规划器"。
不要预判某个 reveal 之后可能出现的新信息来规划后续动作。

每轮输出:
1. 先极其简短地思考(≤2-3句简体中文)，说明你看出的推理要点。
   不要逐格列举棋盘，不要做冗长分析。重点说明: 哪些格确定安全/确定是雷/可双击。
2. 紧接着用工具调用给出当前局面下的安全动作序列(建议 1~5 个)。
   动作必须基于当前棋盘、立刻可尝试。

-------- 批量动作说明 --------
程序接收你的工具调用列表后，会按顺序逐行动执行。
每执行一个动作后，程序都会:
  - 用最新棋盘状态重新校验后续动作是否仍然合法
  - 调用自动插旗机制标记确定雷格
  - 如果发生胜利、踩雷、或棋盘状态发生重大变化(大面积展开/flood)，将自动停止后续动作
  - 如果某个动作被判定为猜测或无进展，也将自动停止后续动作
因此，你不需要担心"某个动作失败之后后续怎么办"，程序侧会处理。
...
- chord(row, col) : 对一个已翻开的数字格执行双击。
    * 当该数字格周围已插旗数 == 该数字时，会一次性翻开其周围所有未翻开且未插旗的格子。
    * 若你标记的雷有误，双击会踩雷导致失败(与人类双击同样规则)。

-------- 辅助机制(程序自动) --------
- 确定性自动插旗: 翻开新区域后，若某数字 N 所在格周围"未翻开格数+已插旗数==N"，程序自动为这些格插旗(必然是雷)。结果直接反映在棋盘文本的 'F' 中。
- 首点保护: 第一步 reveal 永远不会踩雷(其周围3x3区域无雷)。请放心选择中心附近格子。

-------- 策略建议 --------
1. 优先翻开/双击确定性安全格。当某个数字格周围的旗数 == 该数字且仍有未翻开未插旗格时，强烈建议对该数字格调用 chord（一次性展开多个安全格，最省 token）。
2. 数字 N 周围若有 N 个未翻开格且其余邻居已确认，这些未翻开格即为雷 -> 插旗。
3. 找不到确定安全格时，选周围雷数约束最松的格翻开；尽量避免从大数字区域向外硬猜。
4. 如果某个 reveal 属于猜测或单格试探(不是基于严格数字推理)，建议此轮只返回这一个动作，不要再接后续动作。
5. 运用数学/逻辑推理；只有纯逻辑无法推进时才允许概率猜测并简短说明。

如上一轮有动作被跳过，棋盘下方的"上轮结果"注释会告知你，你可以据此调整策略。
请用简体中文思考。"""
    ...
    def _run_loop_stateless(self):
        ...
        while self.running:
            ...
            if not g.first_move_done:
                hint = "\n(第一步: 首点周围永远安全, 推荐中心附近)"
            snapshot = (
                "当前棋盘 (row/col 0-index, '.' 未知, 'F' 旗, 数字 0-8 已知雷数):\n"
                + g.to_text() + "\n" + g.summary()
                + hint + last_summary
            )
            if no_action > 0:
