...
输出要求:
- 每回合必须至少调用一个工具，不能只分析、总结或列计划。
- 可以一次提交多个工具调用，但一批最多 8 个；程序会按返回顺序逐个执行。
- 批量工具调用只适用于确定性安全操作，例如合法 chord、确定安全 reveal、确定雷 toggle_flag。
- 若是概率猜测，只能提交 1 个 reveal 工具调用，不能和其他工具调用放在同一批。
...
- 自然语言说明必须极简；主要通过工具调用表达动作。
- 不要基于 reveal/chord 后可能出现的新信息提前规划太多后续工具调用；不确定时宁可少量提交。
...
class App:
    ...
    def _run_loop(self):
        ...
        while self.running:
            ...
            try:
                result = self.client.request_tools(snapshot)
            except LLMError as e:
            ...
            if thinking and self.enable_reason:
                self._put("thinking", f"[LLM] {thinking[:600]}")
            actions, call_errors = self._tool_calls_to_actions(tool_calls)
            for err in call_errors:
                self._put("error", f"[工具调用格式错误] {err}")
            ...
            if not actions:
                ...
                self._put(
                    "result",
                    f"[模型未提交有效工具调用] 连续无动作 {no_action}/{self.max_no_action}",
                )
                ...
                continue
            no_action = 0
            exec_result = self._execute_action_batch(actions, thinking)
            self._put(
                "result",
    ...
    def _build_state_prompt(self, no_action, remaining):
        ...
        reason_line = (
            'Do not include long natural-language analysis.'
            if not self.enable_reason
            else 'Optional natural-language note must be very short.'
        )
        return (
            "STATE\n"
            f"rows={g.height}\n"
            ...
            f"{g.to_text()}\n\n"
            "TOOL_CALLS_ONLY\n"
            f"Submit 1..{self.max_batch_actions} tool calls in this turn. "
            "Use only reveal/toggle_flag/chord with integer row/col arguments. "
            "If this is a probabilistic guess, submit exactly one reveal tool call. "
            f"{reason_line}\n"
        )
    ...
    def _tool_calls_to_actions(self, tool_calls):
        errors = []
        if not isinstance(tool_calls, list):
            return [], ["tool_calls 必须是数组"]
        limit = self.max_batch_actions if self.enable_batch_actions else 1
        actions = []
        for idx, call in enumerate(tool_calls[:limit]):
            if not isinstance(call, dict):
                errors.append(f"tool_calls[{idx}] 不是 object")
                continue
            name = call.get("name")
            args = call.get("args") or {}
            if name not in ACTION_TYPES:
                errors.append(f"tool_calls[{idx}].name 无效: {name}")
                continue
            if not isinstance(args, dict):
                errors.append(f"tool_calls[{idx}].args 必须是 object")
                continue
            row = args.get("row")
            col = args.get("col")
            if not isinstance(row, int) or not isinstance(col, int):
                errors.append(f"tool_calls[{idx}].row/col 必须是整数")
                continue
            actions.append({"type": name, "row": row, "col": col})
        if len(tool_calls) > limit:
            errors.append(f"工具调用数超过上限 {limit}，已截断")
